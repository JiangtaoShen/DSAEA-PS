function r = krigpls_matern_3_2(theta,d)
% Matern 3/2 correlation function
%=================================
%           d
%   r_i = prod (1 + sqrt(3)*theta_l*m_i) * exp(-sqrt(3)*theta_l*m_i) ;  where, i = 1,...,n, 
%          i=1                                                             and j = 1,...,d           
%
%   m_i = |(x_i_j - x'_i_j)|
%
...
n = size(d,1);
Theta = theta(:).';
term1 = (1 + sqrt(3)*repmat(Theta,n,1).*abs(d)); 
term2 = exp(-sqrt(3)*repmat(Theta,n,1).*abs(d));
