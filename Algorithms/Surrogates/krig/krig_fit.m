function  [dmodel, perf, f] = krig_fit(S, Y, regr, corr, theta0, theta_lb, theta_ub)
% Constrained non-linear least-squares fit of a given correlation model to
% the provided data set and regression model
%
% Call
%   [dmodel, perf] = krig_fit(S, Y, regr, corr, theta0)
%   [dmodel, perf] = krig_fit(S, Y, regr, corr, theta0, lob, upb)
%
% Input
...
%
% Output
% dmodel  : KRIGING model: a struct with the elements
...
if  nargin > 5
    ...
    if  isinf(f)
        warning('Bad parameter region.  Try increasing  upb');
        [dmodel, perf, f] = krig_fit(S, Y);
        return
    end
else
    % Given theta
    ...
    if  isinf(f)
        warning('Bad point.  Try increasing theta0');
        [dmodel, perf, f] = krig_fit(S, Y);
        return
    end
end
