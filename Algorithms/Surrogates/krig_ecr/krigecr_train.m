function [KrigECRmodel,Options] = krigecr_train(xtr, ytr, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while i <= length(varargin)
    ...
    elseif strcmpi(varargin{i},'KRG_NumPCs') || strcmpi(varargin{i},'A') || strcmpi(varargin{i},'PCs') || strcmpi(varargin{i},'PrincipalComponents') || ...
            ...
            strcmpi(varargin{i},'Max_PComps') || strcmpi(varargin{i},'MaxPComps') || strcmpi(varargin{i},'NComps') || strcmpi(varargin{i},'N_Comps')
            
        NumPCs = varargin{i+1};
        i = i+2;
    elseif strcmpi(varargin{i},'KRG_AlphaVal') || strcmpi(varargin{i},'AlphaVal') || strcmpi(varargin{i},'Alpha')
        
        AlphaVal = varargin{i+1};
        i = i+2;
    else
        i = i+1;
    end
...
Options = KrigECRSetOptions(xtr, ytr, FIT_Fn, RegressionModel, CorrelationModel, Theta0, Theta_LB, Theta_UB, NumPCs, AlphaVal);
[KrigECRmodel,State] = KrigECRFitModel(Options);
KrigECRmodel.perf = State.Perf;

return

function Option = KrigECRSetOptions(X, Y, varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Coded and assembled by
%   Ahsanul Habib
%   ahsanul.habib@student.adfa.edu.au
...
if nargin < 2 % Return empty if either X or Y missing
    ...
    Option.FIT_Fn = @krigecr_fit;
    
    Option.RegressionModel  = @krigecr_regpoly0;
    Option.CorrelationModel = @krigecr_corrgauss;
    ...
    Option.NumPCs           = 'optimize';
    Option.AlphaVal         = 'optimize';
end
...
function [Surrogate, State] = KrigECRFitModel(Option)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Coded and assembled by 
%   Ahsanul Habib
%   ahsanul.habib@student.adfa.edu.au
...
switch func2str(Option.FIT_Fn)
    ...
    case {'krigecr_fit','krigecrfit','krigecr','krigpls_fit','krigplsfit','krigpls'}
        State = srgtsFitCreateState(Option);
        if isempty(Option.Theta_LB) % no optimization for theta
            [Surrogate.model, State.Perf, State.FITFuncVal] = krigecr_fit(...
                Option.X, ...
                Option.Y, ...
                Option.RegressionModel, ...
                Option.CorrelationModel, ...
        ...
        Surrogate.model.surro_type = 'krigecr';
end
...
function  [model, perf, f] = krigecr_fit(S, Y, regr, corr, theta0, theta_lb, theta_ub, num_pcs, alphaval)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Parameters
% Input:
% -------------------------------------------------------------------------------------------------
...
%                 default - 'optimize'. In worst case which can n (no. of variables or dimensions)
% alphaval      : If present, then the weight value to balance between PCR and PLS.maximum number 
%                 of principal components
%                 to consider during model building. 
%                 default - 'optimize'. Tries to find value between 0 to 1 with an increment of 0.1.
%
% Output:
% -------------------------------------------------------------------------------------------------
...
%    OptAlpha   : Optimal number of alpha to balance between PCR and PLS
...
DD = S(idmat1,:) - S(idmat2,:);
ij = [idmat1,idmat2];           % initialize matrix with indices
for w = 1:size(Wstar,2)
    D{w} = (S(idmat1,:) - S(idmat2,:)).*repmat(Wstar(:,w)',mzmax,1);  % initialize matrix with distances
end
if  min(sum(abs(DD),2) ) == 0
    SS = S; YY = Y;
...
mzmx = (m^2 - m) / 2;
% Set up  R
td = nan(mzmx,numel(par));
for i = 1:numel(par)
    td(:,i) = feval(par(i).corr, theta(i), par(i).D);
end
r = prod(td,2);
idx = find(r > 0);   o = (1 : m)';
mu = (10+m)*eps;
R = sparse([par(1).ij(idx,1); o], [par(1).ij(idx,2); o], [r(idx); ones(m,1)+mu]);
...
while loop <= length(varargin)
    if strcmpi(varargin{loop},'KRG_NumPCs') || strcmpi(varargin{loop},'A') || strcmpi(varargin{loop},'PCs') || strcmpi(varargin{loop},'PrincipalComponents') || ...
            ...
            strcmpi(varargin{loop},'Num_PCs') || strcmpi(varargin{loop},'Max_PCs') || strcmpi(varargin{loop},'MaxPCs') || ...
            strcmpi(varargin{loop},'Max_PrincipalComponents') || strcmpi(varargin{loop},'MaxPrincipalComponents') || ...
            strcmpi(varargin{loop},'Max_Components') || strcmpi(varargin{loop},'MaxComponents') || ...
            strcmpi(varargin{loop},'Max_PrinComps') || strcmpi(varargin{loop},'MaxPrinComps') || ...
            strcmpi(varargin{loop},'Max_PComps') || strcmpi(varargin{loop},'MaxPComps') || strcmpi(varargin{loop},'NComps') || strcmpi(varargin{loop},'N_Comps')
